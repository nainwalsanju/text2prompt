"""GUI bootstrap and application lifecycle.

This module is imported lazily to avoid loading Cocoa and PyObjC in pure CLI mode.
"""
    ...
    def generate_prompt(self, input_text, mode, popover):
        ...
        def do_generate():
            try:
                ...
                context_id = f"{base_context}{MODE_SEPARATOR}{mode}"
                history = get_history(self.conn, context_id)

                custom_rule = self.config.custom_rules.get(mode, "") if self.config.custom_rules else ""
                full_prompt = self.registry.format_prompt(history, input_text, mode, custom_rule)
                ...
                if self.config.redact_sensitive_info:
...
def start_gui(args: list[str]) -> None:
    """Start Cocoa/PyObjC GUI application event loop."""
